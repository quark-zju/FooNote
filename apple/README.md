...
默认窄条窗口（340 × 760），顶部树状笔记、底部纯文本编辑器；中间分割栏
可以上下拖动。窗口最小宽度 260。系统可能恢复此前的窗口尺寸，可手动缩窄。
...
编辑写入后端内存，不会每次点击都触发 Git push；没有定时自动保存。
...
尚无加密解锁、挂载地址编辑/卸载界面、桌面停靠、跨应用拖拽和树操作撤销。
